...
function WhatsAppSection() {
    const [activeRole, setActiveRole] = (0, react_1.useState)(0);
    ...
    const openWhatsApp = (message) => {
        window.open(`https://wa.me/${TWILIO_NUMBER}?text=${encodeURIComponent(message)}`, '_blank');
    };
    const showChat = (label) => {
        setAnimating(true);
        setActiveChat(label);
        setTimeout(() => setAnimating(false), 300);
    };
    const chatMessages = activeChat ? (DEMO_CHATS[activeChat] || []) : [];
    return (<section className="py-20 px-4" style={{ background: '#F0FDFA' }}>
      <div className="max-w-6xl mx-auto">

        {/* Section Header */}
        <div className="text-center mb-12">
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full text-sm font-bold mb-4" style={{ background: '#CCFBF1', color: '#0D9488', border: '1.5px solid #99F6E4' }}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
          ...
          <div>
            {/* Role Switcher */}
            <div className="flex gap-2 mb-6 p-1.5 rounded-2xl" style={{ background: 'white', border: '1.5px solid #E0F2FE' }}>
              {FLOWS.map((f, i) => (<button key={f.role} onClick={() => { setActiveRole(i); setActiveChat(null); }} className="flex-1 py-2.5 rounded-xl text-sm font-bold transition-all flex items-center justify-center gap-2" style={{
            ...
            <div className="grid grid-cols-2 gap-3 mb-6">
              {flow.actions.map((action) => (<button key={action.label} onClick={() => showChat(action.label)} className="p-4 rounded-2xl text-left transition-all hover:shadow-md hover:-translate-y-0.5 bg-white" style={{
            ...
            {/* Connect Box */}
            <div className="p-5 rounded-3xl" style={{ background: 'linear-gradient(135deg, #25D366, #128C7E)', boxShadow: '0 8px 32px rgba(37,211,102,0.3)' }}>
              ...
              <div className="flex gap-4 items-center mb-4">
                <div className="p-2 rounded-xl bg-white flex-shrink-0">
                  <img src={`https://api.qrserver.com/v1/create-qr-code/?size=90x90&data=https://wa.me/${TWILIO_NUMBER}?text=HELP&bgcolor=ffffff&color=128C7E`} alt="WhatsApp QR" className="w-20 h-20 rounded-lg"/>
                </div>
              ...
              <button onClick={() => openWhatsApp('HELP')} className="w-full py-3 rounded-2xl font-bold text-sm transition-all hover:scale-[1.02] flex items-center justify-center gap-2" style={{ background: 'white', color: '#128C7E' }}>
                <span className="text-lg">💬</span>
                Open WhatsApp Now →
              </button>
            </div>
          ...
          {/* RIGHT — Chat Preview */}
          <div className="rounded-3xl overflow-hidden" style={{ border: '1.5px solid #E0F2FE', boxShadow: '0 8px 40px rgba(0,0,0,0.08)', background: 'white' }}>
            ...
            {/* Chat Area */}
            <div className="p-4 space-y-3 min-h-80 max-h-96 overflow-y-auto" style={{ background: '#ECE5DD' }}>

              {!activeChat ? (<div className="flex flex-col items-center justify-center h-64 text-center">
                  ...
                  {chatMessages.length === 0 ? (<div className="flex flex-col items-center justify-center h-64 text-center">
                      <div className="text-4xl mb-3">🤖</div>
                      <div className="text-sm font-semibold" style={{ color: '#475569' }}>
                        This command opens WhatsApp directly
                      </div>
                      <button onClick={() => { var _a; return openWhatsApp(((_a = flow.actions.find(a => a.label === activeChat)) === null || _a === void 0 ? void 0 : _a.message) || 'HELP'); }} className="mt-3 px-4 py-2 rounded-xl text-white text-sm font-semibold" style={{ background: '#25D366' }}>
                        Try it on WhatsApp →
                      </button>
                    </div>) : (chatMessages.map((msg, i) => (<div key={i} className={`flex ${msg.from === 'user' ? 'justify-end' : 'justify-start'}`} style={{ animationDelay: `${i * 0.1}s` }}>
                    ...
                    whiteSpace: 'pre-line',
                          ...
                          {msg.from === 'bot' && (<div className="text-xs font-bold mb-1" style={{ color: '#128C7E' }}>
                              CarePulse AI 🏥
                            </div>)}
                          {msg.text}
            ...
            {/* Try Button */}
            {activeChat && chatMessages.length > 0 && (<div className="p-4 border-t" style={{ borderColor: '#F1F5F9' }}>
                <button onClick={() => { var _a; return openWhatsApp(((_a = flow.actions.find(a => a.label === activeChat)) === null || _a === void 0 ? void 0 : _a.message) || 'HELP'); }} className="w-full py-3 rounded-2xl text-white font-bold text-sm transition-all hover:shadow-lg flex items-center justify-center gap-2" style={{ background: 'linear-gradient(135deg, #25D366, #128C7E)' }}>
                  <span className="text-lg">📲</span>
        ...
        ].map((stat) => (<div key={stat.label} className="p-5 rounded-2xl text-center bg-white" style={{ border: '1.5px solid #E0F2FE', boxShadow: '0 2px 12px rgba(0,0,0,0.04)' }}>
...
exports.default = WhatsAppSection;
//# sourceMappingURL=WhatsAppSection.js.map