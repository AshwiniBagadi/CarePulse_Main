...
async function setSession(phone, update) {
    const current = await getSession(phone);
    await db.collection('whatsapp_sessions').doc(phone).set(Object.assign(Object.assign(Object.assign({}, current), update), { lastActivity: Date.now() }), { merge: true });
}
...
// ─── Menu Builders ────────────────────────────────────────────────────────────
const mainMenu = () => `👋 *Welcome to CarePulse AI!*
...
const opdConfirmMsg = (d) => `📋 *Confirm OPD Appointment:*
...
const bedConfirmMsg = (d) => `📋 *Confirm Bed Booking:*
...
async function handleIncomingWhatsApp(fromRaw, bodyText) {
    ...
    if (session.step === 'MAIN_MENU') {
        if (text === '1') {
            await setSession(phone, { step: 'OPD_HOSPITAL', mode: 'opd' });
            return sendWhatsApp(phone, hospitalMenu());
        }
        if (text === '2') {
            await setSession(phone, { step: 'BED_HOSPITAL', mode: 'bed' });
            return sendWhatsApp(phone, hospitalMenu());
        }
        if (text === '3')
    ...
    if (session.step === 'OPD_HOSPITAL') {
        ...
        if (isNaN(idx) || idx < 0 || idx >= HOSPITALS.length)
            return sendWhatsApp(phone, `❌ Reply 1–${HOSPITALS.length}.\n\n${hospitalMenu()}`);
        const h = HOSPITALS[idx];
        await setSession(phone, { step: 'OPD_DEPARTMENT', data: Object.assign(Object.assign({}, session.data), { hospitalId: h.id, hospitalName: h.name }) });
    ...
    if (session.step === 'OPD_DEPARTMENT') {
        ...
        if (isNaN(idx) || idx < 0 || idx >= DEPARTMENTS.length)
            return sendWhatsApp(phone, `❌ Reply 1–${DEPARTMENTS.length}.\n\n${departmentMenu()}`);
        const d = DEPARTMENTS[idx];
        await setSession(phone, { step: 'OPD_DOCTOR', data: Object.assign(Object.assign({}, session.data), { deptId: d.id, deptName: d.name }) });
    ...
    if (session.step === 'OPD_DOCTOR') {
        ...
        if (isNaN(idx) || idx < 0 || idx >= docs.length)
            return sendWhatsApp(phone, `❌ Reply 1–${docs.length}.\n\n${doctorMenu(session.data.deptId)}`);
        const doc = docs[idx];
        await setSession(phone, { step: 'OPD_SLOT', data: Object.assign(Object.assign({}, session.data), { doctorId: doc.id, doctorName: doc.name, doctorIdx: String(idx) }) });
    ...
    if (session.step === 'OPD_SLOT') {
        ...
        await setSession(phone, { step: 'OPD_DATE', data: Object.assign(Object.assign({}, session.data), { slot: doc.slots[sIdx] }) });
        return sendWhatsApp(phone, `📅 *Enter Appointment Date:*\n\nFormat: *DD-MM-YYYY*\nExample: 15-03-2026`);
    }
    // ── OPD: Date ──────────────────────────────────────────────────────────────
    if (session.step === 'OPD_DATE') {
        if (!/^\d{2}-\d{2}-\d{4}$/.test(text))
            return sendWhatsApp(phone, `❌ Use format *DD-MM-YYYY*\nExample: 15-03-2026`);
        await setSession(phone, { step: 'OPD_PATIENT_NAME', data: Object.assign(Object.assign({}, session.data), { date: text }) });
    ...
    if (session.step === 'OPD_PATIENT_NAME') {
        ...
        await setSession(phone, { step: 'OPD_SYMPTOMS', data: Object.assign(Object.assign({}, session.data), { patientName: text }) });
        return sendWhatsApp(phone, `🤒 *Describe Symptoms:*\n\n(e.g. fever, chest pain)\nOr reply *SKIP*`);
    }
    // ── OPD: Symptoms ──────────────────────────────────────────────────────────
    if (session.step === 'OPD_SYMPTOMS') {
        const newData = Object.assign(Object.assign({}, session.data), { symptoms: upper === 'SKIP' ? 'Not specified' : text });
    ...
    if (session.step === 'BED_HOSPITAL') {
        ...
        if (isNaN(idx) || idx < 0 || idx >= HOSPITALS.length)
            return sendWhatsApp(phone, `❌ Reply 1–${HOSPITALS.length}.\n\n${hospitalMenu()}`);
        const h = HOSPITALS[idx];
        await setSession(phone, { step: 'BED_TYPE', data: Object.assign(Object.assign({}, session.data), { hospitalId: h.id, hospitalName: h.name }) });
    ...
    if (session.step === 'BED_TYPE') {
        ...
        if (isNaN(idx) || idx < 0 || idx >= BED_TYPES.length)
            return sendWhatsApp(phone, `❌ Reply 1–${BED_TYPES.length}.\n\n${bedMenu()}`);
        const bed = BED_TYPES[idx];
        await setSession(phone, { step: 'BED_PATIENT_NAME', data: Object.assign(Object.assign({}, session.data), { bedId: bed.id, bedName: bed.name, bedPrice: String(bed.price) }) });
        return sendWhatsApp(phone, `👤 *Enter Patient's Full Name:*`);
    }
    // ── BED: Patient Name ──────────────────────────────────────────────────────
    if (session.step === 'BED_PATIENT_NAME') {
        if (text.length < 3)
            return sendWhatsApp(phone, `❌ Please enter a valid name.`);
        await setSession(phone, { step: 'BED_MOBILE', data: Object.assign(Object.assign({}, session.data), { patientName: text }) });
        return sendWhatsApp(phone, `📱 *Enter Patient's Mobile Number:*\n(10 digits, no country code)`);
    }
    // ── BED: Mobile ────────────────────────────────────────────────────────────
    if (session.step === 'BED_MOBILE') {
        if (!/^\d{10}$/.test(text))
            return sendWhatsApp(phone, `❌ Enter 10 digits.\nExample: 9876543210`);
        await setSession(phone, { step: 'BED_GENDER', data: Object.assign(Object.assign({}, session.data), { mobile: text }) });
    ...
    if (session.step === 'BED_GENDER') {
        ...
        if (isNaN(idx) || idx < 0 || idx > 2)
            return sendWhatsApp(phone, `❌ Reply 1, 2 or 3.`);
        await setSession(phone, { step: 'BED_AADHAAR', data: Object.assign(Object.assign({}, session.data), { gender: ['Male', 'Female', 'Other'][idx] }) });
        return sendWhatsApp(phone, `🪪 *Enter Aadhaar Number:*\n(12 digits)\nOr reply *SKIP*`);
    }
    // ── BED: Aadhaar ───────────────────────────────────────────────────────────
    if (session.step === 'BED_AADHAAR') {
        if (upper !== 'SKIP' && !/^\d{12}$/.test(text))
            return sendWhatsApp(phone, `❌ Aadhaar must be 12 digits or reply *SKIP*.`);
        await setSession(phone, { step: 'BED_DOCTOR', data: Object.assign(Object.assign({}, session.data), { aadhaar: upper === 'SKIP' ? 'Not provided' : text }) });
        return sendWhatsApp(phone, `👨‍⚕️ *Doctor who advised admission:*\n(Or reply *SKIP*)`);
    }
    // ── BED: Doctor ────────────────────────────────────────────────────────────
    if (session.step === 'BED_DOCTOR') {
        await setSession(phone, { step: 'BED_REASON', data: Object.assign(Object.assign({}, session.data), { doctorName: upper === 'SKIP' ? 'Not specified' : text }) });
    ...
    if (session.step === 'BED_REASON') {
        ...
        if (isNaN(idx) || idx < 0 || idx >= reasons.length)
            return sendWhatsApp(phone, `❌ Reply 1–6.`);
        await setSession(phone, { step: 'BED_DATE', data: Object.assign(Object.assign({}, session.data), { reason: reasons[idx] }) });
    ...
    if (session.step === 'BED_DATE') {
        if (!/^\d{2}-\d{2}-\d{4}$/.test(text))
            return sendWhatsApp(phone, `❌ Use format *DD-MM-YYYY*`);
        const newData = Object.assign(Object.assign({}, session.data), { date: text });
...
exports.notifyAppointmentConfirmed = functions.firestore
    ...
    if (appt.source !== 'whatsapp' && appt.patientPhone) {
        await sendWhatsApp(appt.patientPhone, `🏥 *Appointment Confirmed — CarePulse AI*
