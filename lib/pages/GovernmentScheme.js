...
function GovernmentSchemes() {
    ...
    const [eligible, setEligible] = (0, react_1.useState)([]);
    ...
    const checkEligibility = () => {
        const results = [];
        if (filters.income === 'below_2.5')
            results.push('pmjay', 'ayushman', 'mjpjay');
        if (filters.income === 'below_5')
            results.push('ayushman');
        if (filters.income === 'govt_employee')
            results.push('cghs');
        if (filters.income === 'salaried')
            results.push('esic');
        if (results.length === 0)
            results.push('ayushman');
        setEligible(results);
    ...
    const filteredSchemes = SCHEMES.filter(s => s.name.toLowerCase().includes(search.toLowerCase()) ||
    ...
    return (<div style={{ background: '#EFF6FF', minHeight: '100vh' }}>
      ...
      <div className="bg-white sticky top-0 z-50 px-6 py-4 flex items-center gap-4" style={{ borderBottom: '1.5px solid #E0F2FE', boxShadow: '0 2px 8px rgba(0,0,0,0.04)' }}>
        <button onClick={() => navigate('/dashboard')} className="text-sm font-medium" style={{ color: '#64748B' }}>
          ← Dashboard
        </button>
      ...
      {/* Hero Banner */}
      <div className="relative overflow-hidden mx-4 mt-6 rounded-3xl" style={{ background: 'linear-gradient(135deg, #BAE6FD, #CCFBF1)', minHeight: '200px' }}>
        <div className="absolute inset-0 opacity-10" style={{ backgroundImage: 'radial-gradient(circle at 20% 50%, #0D9488 0%, transparent 50%), radial-gradient(circle at 80% 50%, #0EA5E9 0%, transparent 50%)' }}/>
        <div className="relative flex items-center justify-between px-8 py-8">
          <div className="flex-1">
            ...
            <div className="flex gap-3 mt-4">
              <div className="px-4 py-2 rounded-xl text-sm font-semibold" style={{ background: 'white', color: '#0D9488' }}>
                5 Active Schemes
              </div>
              <div className="px-4 py-2 rounded-xl text-sm font-semibold" style={{ background: 'rgba(255,255,255,0.6)', color: '#475569' }}>
                Auto-Detection ✨
              </div>
            </div>
          </div>
          <div className="hidden md:block">
            <img src="https://images.unsplash.com/photo-1582750433449-648ed127bb54?w=300&q=80" alt="Doctors" className="w-48 h-48 object-cover rounded-2xl" style={{ border: '3px solid white', boxShadow: '0 8px 32px rgba(0,0,0,0.12)' }}/>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4 mx-4 mt-4">
        <div className="p-5 rounded-2xl flex items-center gap-4 cursor-pointer hover:shadow-md transition-all" style={{ background: '#BAE6FD', border: '1.5px solid #7DD3FC' }}>
        ...
        <div className="p-5 rounded-2xl flex items-center gap-4 cursor-pointer hover:shadow-md transition-all" style={{ background: '#CCFBF1', border: '1.5px solid #6EE7B7' }}>
          <div className="w-12 h-12 rounded-xl flex items-center justify-center text-2xl bg-white">📋</div>
          <div>
      ...
      <div className="max-w-3xl mx-auto px-4 py-6 space-y-6">
        ...
        <div className="rounded-3xl p-6 bg-white" style={{ border: '1.5px solid #E0F2FE', boxShadow: '0 2px 16px rgba(0,0,0,0.06)' }}>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-3">
            <div>
              ...
              <select name="state" value={filters.state} onChange={handleFilter} className="w-full px-3 py-2.5 rounded-xl outline-none text-sm" style={{ background: '#F8FAFC', border: '1.5px solid #E2E8F0', color: '#0F172A' }}>
            ...
            <div>
              ...
              <select name="age" value={filters.age} onChange={handleFilter} className="w-full px-3 py-2.5 rounded-xl outline-none text-sm" style={{ background: '#F8FAFC', border: '1.5px solid #E2E8F0', color: '#0F172A' }}>
            ...
            <div>
              ...
              <select name="income" value={filters.income} onChange={handleFilter} className="w-full px-3 py-2.5 rounded-xl outline-none text-sm" style={{ background: '#F8FAFC', border: '1.5px solid #E2E8F0', color: '#0F172A' }}>
            ...
            <div>
              ...
              <select name="gender" value={filters.gender} onChange={handleFilter} className="w-full px-3 py-2.5 rounded-xl outline-none text-sm" style={{ background: '#F8FAFC', border: '1.5px solid #E2E8F0', color: '#0F172A' }}>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3 mb-5">
            <div>
              ...
              <select name="familySize" value={filters.familySize} onChange={handleFilter} className="w-full px-3 py-2.5 rounded-xl outline-none text-sm" style={{ background: '#F8FAFC', border: '1.5px solid #E2E8F0', color: '#0F172A' }}>
            ...
            <div>
              ...
              <select name="maritalStatus" value={filters.maritalStatus} onChange={handleFilter} className="w-full px-3 py-2.5 rounded-xl outline-none text-sm" style={{ background: '#F8FAFC', border: '1.5px solid #E2E8F0', color: '#0F172A' }}>
            ...
            <div>
              ...
              <select name="healthProblem" value={filters.healthProblem} onChange={handleFilter} className="w-full px-3 py-2.5 rounded-xl outline-none text-sm" style={{ background: '#F8FAFC', border: '1.5px solid #E2E8F0', color: '#0F172A' }}>
          ...
          <div className="flex gap-3">
            <button onClick={checkEligibility} className="flex-1 py-3 rounded-xl text-white font-semibold transition-all hover:shadow-lg" style={{ background: 'linear-gradient(135deg, #0D9488, #0EA5E9)' }}>
              ✨ To all users
            </button>
            <button onClick={checkEligibility} className="flex-1 py-3 rounded-xl font-semibold transition-all hover:shadow-md" style={{ background: '#CCFBF1', color: '#0D9488', border: '1.5px solid #0D9488' }}>
              🏥 To all hospitals
            </button>
          </div>

          {checked && eligible.length > 0 && (<div className="mt-4 p-4 rounded-2xl" style={{ background: '#CCFBF1', border: '1.5px solid #0D9488' }}>
              ...
              <div className="flex flex-wrap gap-2">
                ...
                return s ? (<span key={id} className="px-3 py-1 rounded-full text-xs font-semibold" style={{ background: s.bg, color: s.color }}>
                      {s.name}
                    </span>) : null;
            })}
              </div>
        ...
        <div className="relative">
          ...
          <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search By Scheme Name..." className="w-full pl-11 pr-4 py-3 rounded-2xl outline-none text-sm bg-white shadow-sm" style={{ border: '1.5px solid #E2E8F0', color: '#0F172A' }}/>
        </div>

        {/* Scheme Cards */}
        <div className="space-y-4">
          {filteredSchemes.map((scheme, idx) => (<div key={scheme.id} className="rounded-3xl bg-white overflow-hidden transition-all hover:shadow-lg" style={{
              ...
              <div className="p-5 flex items-center gap-4">
                <div className="w-14 h-14 rounded-2xl flex items-center justify-center text-2xl flex-shrink-0" style={{ background: scheme.bg }}>
                  {scheme.icon}
                </div>
                <div className="flex-1">
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    <span className="px-3 py-0.5 rounded-full text-xs font-bold" style={{ background: scheme.bg, color: scheme.color }}>
                      {scheme.name}
                    </span>
                    {eligible.includes(scheme.id) && checked && (<span className="px-3 py-0.5 rounded-full text-xs font-bold" style={{ background: '#CCFBF1', color: '#0D9488' }}>
                        ✅ You're Eligible!
                      </span>)}
                  </div>
            ...
            ].map((stat, i) => (<div key={stat.label} className="py-3 text-center" style={{ borderRight: i < 2 ? '1px solid #F1F5F9' : 'none' }}>
              ...
              {/* Expand / Collapse */}
              <button onClick={() => setExpanded(expanded === scheme.id ? null : scheme.id)} className="w-full px-5 py-3 flex items-center justify-between text-sm font-semibold transition-all" style={{ color: scheme.color }}>
                <span>{expanded === scheme.id ? 'Hide Details ▲' : 'View Details ▼'}</span>
              </button>

              {/* Expanded Details */}
              {expanded === scheme.id && (<div className="px-5 pb-5 space-y-4 border-t" style={{ borderColor: '#F1F5F9' }}>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                    <div className="p-4 rounded-2xl" style={{ background: scheme.bg }}>
                      ...
                      <ul className="space-y-1">
                        {scheme.eligibility.map((item) => (<li key={item} className="text-sm flex items-start gap-2" style={{ color: '#475569' }}>
                    ...
                    <div className="p-4 rounded-2xl" style={{ background: '#F8FAFC', border: '1.5px solid #E2E8F0' }}>
                      ...
                      <ul className="space-y-1">
                        {scheme.documents.map((doc) => (<li key={doc} className="text-sm flex items-start gap-2" style={{ color: '#475569' }}>
                  ...
                  <a href={scheme.applyLink} target="_blank" rel="noopener noreferrer" className="block w-full py-3 rounded-2xl text-white font-semibold text-center transition-all hover:shadow-lg hover:scale-[1.02]" style={{ background: `linear-gradient(135deg, ${scheme.color}, #0EA5E9)` }}>
...
exports.default = GovernmentSchemes;
//# sourceMappingURL=GovernmentScheme.js.map