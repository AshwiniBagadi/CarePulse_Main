...
function AdminDashboard() {
    ...
    const [admin, setAdmin] = (0, react_1.useState)(null);
    ...
    return (<div style={{ background: '#EFF6FF', minHeight: '100vh' }}>
      ...
      <nav className="bg-white sticky top-0 z-50 px-6 py-4 flex items-center justify-between" style={{ borderBottom: '1.5px solid #E0F2FE', boxShadow: '0 2px 8px rgba(0,0,0,0.04)' }}>
        <div className="flex items-center gap-3">
          ...
          <span className="text-sm px-3 py-1 rounded-full font-medium" style={{ background: '#FEF3C7', color: '#F59E0B' }}>
            Admin Portal
          </span>
        </div>
        <div className="flex items-center gap-4">
          ...
          <div className="w-9 h-9 rounded-full flex items-center justify-center font-bold text-white text-sm" style={{ background: 'linear-gradient(135deg, #F59E0B, #EF4444)' }}>
            {((_a = admin === null || admin === void 0 ? void 0 : admin.name) === null || _a === void 0 ? void 0 : _a[0]) || 'A'}
          </div>
          <button onClick={() => { (0, auth_1.signOut)(config_1.auth); navigate('/'); }} className="px-4 py-2 rounded-xl text-sm font-medium" style={{ color: '#EF4444', border: '1.5px solid #FCA5A5' }}>
            Logout
          </button>
        </div>
      </nav>
      ...
      {/* Tabs */}
      <div className="bg-white border-b px-6 overflow-x-auto" style={{ borderColor: '#E0F2FE' }}>
        <div className="flex gap-1 max-w-6xl mx-auto">
          {tabs.map((tab) => (<button key={tab} onClick={() => setActiveTab(tab)} className="px-5 py-4 text-sm font-semibold capitalize transition-all whitespace-nowrap" style={{
      ...
      <div className="max-w-6xl mx-auto px-6 py-8">
        ...
        {activeTab === 'overview' && (<div className="space-y-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {KPI_CARDS.map((kpi) => (<div key={kpi.label} className="p-6 rounded-3xl text-white relative overflow-hidden" style={{ background: kpi.gradient, boxShadow: '0 8px 32px rgba(0,0,0,0.12)' }}>
                  ...
                  <div className="text-3xl font-bold mb-1" style={{ fontFamily: 'Plus Jakarta Sans' }}>
                    {kpi.value}
                  </div>
                  <div className="text-white/70 text-sm mb-2">{kpi.label}</div>
                  <div className="text-white/90 text-xs font-semibold px-2 py-1 rounded-full inline-block" style={{ background: 'rgba(255,255,255,0.2)' }}>
                    {kpi.change}
                  </div>
                </div>))}
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="p-6 rounded-3xl bg-white" style={{ border: '1.5px solid #E0F2FE' }}>
                ...
                <recharts_1.ResponsiveContainer width="100%" height={200}>
                  <recharts_1.LineChart data={hourlyData}>
                    <recharts_1.XAxis dataKey="time" tick={{ fontSize: 11, fill: '#94A3B8' }} axisLine={false} tickLine={false}/>
                    <recharts_1.YAxis tick={{ fontSize: 11, fill: '#94A3B8' }} axisLine={false} tickLine={false}/>
                    <recharts_1.Tooltip contentStyle={{ borderRadius: '12px', border: '1px solid #E0F2FE', fontSize: 12 }}/>
                    <recharts_1.Line type="monotone" dataKey="patients" stroke="#0D9488" strokeWidth={3} dot={false}/>
              ...
              <div className="p-6 rounded-3xl bg-white" style={{ border: '1.5px solid #E0F2FE' }}>
                ...
                <div className="flex items-center gap-4">
                  <recharts_1.ResponsiveContainer width="60%" height={200}>
                    <recharts_1.PieChart>
                      <recharts_1.Pie data={statusData} cx="50%" cy="50%" innerRadius={50} outerRadius={80} dataKey="value">
                        {statusData.map((entry, index) => (<recharts_1.Cell key={index} fill={entry.color}/>))}
                      </recharts_1.Pie>
                      <recharts_1.Tooltip contentStyle={{ borderRadius: '12px', fontSize: 12 }}/>
                    </recharts_1.PieChart>
                  </recharts_1.ResponsiveContainer>
                  <div className="space-y-2">
                    {statusData.map((item) => (<div key={item.name} className="flex items-center gap-2">
            ...
            <div className="p-6 rounded-3xl bg-white" style={{ border: '1.5px solid #E0F2FE' }}>
              ...
              <recharts_1.ResponsiveContainer width="100%" height={200}>
                <recharts_1.BarChart data={deptData}>
                  <recharts_1.XAxis dataKey="dept" tick={{ fontSize: 11, fill: '#94A3B8' }} axisLine={false} tickLine={false}/>
                  <recharts_1.YAxis tick={{ fontSize: 11, fill: '#94A3B8' }} axisLine={false} tickLine={false}/>
                  <recharts_1.Tooltip contentStyle={{ borderRadius: '12px', border: '1px solid #E0F2FE', fontSize: 12 }}/>
                  <recharts_1.Bar dataKey="patients" fill="#0D9488" radius={[8, 8, 0, 0]}/>
                </recharts_1.BarChart>
              </recharts_1.ResponsiveContainer>
            </div>
          </div>)}
        ...
        {activeTab === 'queue' && (<div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {DEPARTMENTS.map((dept) => (<div key={dept.name} className="p-5 rounded-2xl bg-white" style={{ border: `2px solid ${crowdColor(dept.crowd)}30`, boxShadow: '0 2px 12px rgba(0,0,0,0.04)' }}>
                  <div className="flex items-center justify-between mb-3">
                    <h3 className="font-bold text-sm" style={{ color: '#0F172A' }}>{dept.name}</h3>
                    <div className="flex items-center gap-1.5 px-2 py-1 rounded-full text-xs font-semibold" style={{ background: crowdBg(dept.crowd), color: crowdColor(dept.crowd) }}>
                      <div className="w-1.5 h-1.5 rounded-full animate-pulse" style={{ background: crowdColor(dept.crowd) }}/>
                      {dept.crowd}
                    </div>
                  </div>
                ...
                ].map((stat) => (<div key={stat.label} className="text-center p-2 rounded-xl" style={{ background: '#F8FAFC' }}>
        ...
        {/* DOCTORS TAB */}
        {activeTab === 'doctors' && (<div className="space-y-6">
            <div className="flex items-center justify-between">
              <h2 className="text-xl font-bold" style={{ color: '#0F172A' }}>👨‍⚕️ Doctor Management</h2>
              <button className="px-5 py-2 rounded-xl text-white text-sm font-semibold" style={{ background: 'linear-gradient(135deg, #0D9488, #0EA5E9)' }}>
                + Add Doctor
              </button>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {DOCTORS.map((doc) => (<div key={doc.name} className="p-5 rounded-2xl bg-white flex items-center gap-4" style={{ border: '1.5px solid #E0F2FE', boxShadow: '0 2px 12px rgba(0,0,0,0.04)' }}>
        ...
        {activeTab === 'analytics' && (<div className="space-y-6">
            ...
            ].map((item) => (<div key={item.label} className="p-5 rounded-2xl bg-white text-center" style={{ border: '1.5px solid #E0F2FE' }}>
            ...
            <div className="p-6 rounded-3xl bg-white" style={{ border: '1.5px solid #E0F2FE' }}>
              ...
              <recharts_1.ResponsiveContainer width="100%" height={250}>
                <recharts_1.BarChart data={deptData}>
                  <recharts_1.XAxis dataKey="dept" tick={{ fontSize: 12, fill: '#94A3B8' }} axisLine={false} tickLine={false}/>
                  <recharts_1.YAxis tick={{ fontSize: 12, fill: '#94A3B8' }} axisLine={false} tickLine={false}/>
                  <recharts_1.Tooltip contentStyle={{ borderRadius: '12px', border: '1px solid #E0F2FE' }}/>
                  <recharts_1.Bar dataKey="patients" radius={[8, 8, 0, 0]}>
                    {deptData.map((_, index) => (<recharts_1.Cell key={index} fill={index % 2 === 0 ? '#0D9488' : '#0EA5E9'}/>))}
                  </recharts_1.Bar>
                </recharts_1.BarChart>
              </recharts_1.ResponsiveContainer>
            </div>
        ...
        {/* SCHEMES TAB */}
        {activeTab === 'schemes' && (<div className="space-y-6">
            ...
            ].map((item) => (<div key={item.scheme} className="p-5 rounded-2xl bg-white" style={{ border: `1.5px solid ${item.color}30`, boxShadow: '0 2px 12px rgba(0,0,0,0.04)' }}>
                  <div className="w-10 h-10 rounded-xl flex items-center justify-center text-lg mb-3" style={{ background: item.bg }}>
                    🏛️
                  </div>
