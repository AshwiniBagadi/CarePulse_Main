...
function BookAppointment() {
    ...
    const [mode, setMode] = (0, react_1.useState)('opd');
    const [search, setSearch] = (0, react_1.useState)('');
    const [symptoms, setSymptoms] = (0, react_1.useState)('');
    const [suggested, setSuggested] = (0, react_1.useState)(null);
    const [loading, setLoading] = (0, react_1.useState)(false);
    const [success, setSuccess] = (0, react_1.useState)(false);
    const [selected, setSelected] = (0, react_1.useState)({
        hospital: '', department: '', doctor: '', slot: '', date: '', bed: '',
    });
    const filteredHospitals = HOSPITALS.filter(h => h.name.toLowerCase().includes(search.toLowerCase()) ||
    ...
    const handleBook = async () => {
        ...
        try {
            ...
            const appointmentRef = await (0, firestore_1.addDoc)((0, firestore_1.collection)(config_1.db, "appointments"), {
                patientId: user.uid,
    ...
    if (success) {
        return (<div className="min-h-screen flex items-center justify-center" style={{ background: '#F0FDFA' }}>
    ...
    return (<div className="min-h-screen" style={{ background: '#F8FAFC' }}>
      ...
      <div className="sticky top-0 z-10 flex items-center gap-4 px-6 py-4 bg-white shadow-sm border-b border-gray-100">
        <button onClick={() => step > 1 ? setStep(step - 1) : navigate('/dashboard')} className="flex items-center gap-1 text-sm font-medium transition-colors" style={{ color: '#64748B' }}>
          ← Back
        </button>
        ...
        <div className="ml-auto flex rounded-xl p-1 gap-1" style={{ background: '#F1F5F9' }}>
          {[{ key: 'opd', label: '🩺 OPD' }, { key: 'bed', label: '🛏️ Bed Booking' }].map((m) => (<button key={m.key} onClick={() => { setMode(m.key); setStep(1); }} className="px-4 py-1.5 rounded-lg text-sm font-semibold transition-all" style={{
      ...
      <div className="bg-white border-b border-gray-100 px-6 py-4">
        <div className="flex items-center justify-center gap-2 max-w-lg mx-auto">
          {steps.map((label, i) => (<div key={label} className="flex items-center gap-2">
              <div className="flex flex-col items-center">
                <div className="w-9 h-9 rounded-full flex items-center justify-center text-sm font-bold transition-all shadow-sm" style={{
                ...
                <span className="text-xs mt-1 font-medium hidden md:block" style={{ color: step === i + 1 ? '#0D9488' : '#94A3B8' }}>
                  {label}
                </span>
              </div>
              {i < steps.length - 1 && (<div className="w-10 md:w-20 h-0.5 mb-4 rounded-full transition-all" style={{ background: step > i + 1 ? '#0D9488' : '#E2E8F0' }}/>)}
            </div>))}
        </div>
      </div>
      ...
      <div className="max-w-2xl mx-auto px-4 py-8 pb-16">
        ...
        {step === 1 && (<div>
            ...
            <div className="relative mb-6">
              ...
              <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search by hospital name or city..." className="w-full pl-11 pr-4 py-3 rounded-2xl outline-none text-sm shadow-sm" style={{ background: 'white', border: '1.5px solid #E2E8F0', color: '#0F172A' }}/>
            </div>
            <div className="space-y-4">
              {filteredHospitals.map((h) => (<button key={h.id} onClick={() => { setSelected(Object.assign(Object.assign({}, selected), { hospital: h.id })); setStep(2); }} className="w-full rounded-2xl overflow-hidden text-left transition-all hover:shadow-lg bg-white" style={{ border: `2px solid ${selected.hospital === h.id ? '#0D9488' : '#F1F5F9'}`, boxShadow: '0 2px 12px rgba(0,0,0,0.06)' }}>
                  <div className="relative h-44 overflow-hidden">
                    <img src={h.photo} alt={h.name} className="w-full h-full object-cover"/>
                    <div className="absolute inset-0" style={{ background: 'linear-gradient(to top, rgba(0,0,0,0.5), transparent)' }}/>
                    <div className="absolute top-3 left-3 flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold" style={{ background: crowdBg(h.crowd), color: crowdColor(h.crowd) }}>
                      <div className="w-1.5 h-1.5 rounded-full animate-pulse" style={{ background: crowdColor(h.crowd) }}/>
                      {h.crowd} crowd
                    </div>
                    {mode === 'bed' && (<div className="absolute top-3 right-3 px-3 py-1 rounded-full text-xs font-bold" style={{ background: h.beds > 0 ? '#ECFDF5' : '#FEF2F2', color: h.beds > 0 ? '#0D9488' : '#EF4444' }}>
                        {h.beds > 0 ? `${h.beds} beds free` : 'No beds'}
                      </div>)}
                    <div className="absolute bottom-3 right-3 px-3 py-1 rounded-full text-xs font-semibold" style={{ background: 'rgba(0,0,0,0.5)', color: 'white' }}>
                      ⏱ ~{h.wait} min wait
                    </div>
                  </div>
                  <div className="p-4 flex items-center justify-between">
                    ...
                    <div className="w-9 h-9 rounded-full flex items-center justify-center" style={{ background: 'linear-gradient(135deg, #0D9488, #0EA5E9)' }}>
        ...
        {step === 2 && mode === 'opd' && (<div>
            ...
            <div className="p-4 rounded-2xl mb-6 bg-white shadow-sm" style={{ border: '1.5px solid #E9D5FF' }}>
              <div className="flex items-center gap-2 mb-3">
                <span className="text-lg">🤖</span>
                <span className="font-semibold text-sm" style={{ color: '#7C3AED' }}>AI Symptom Assistant</span>
              </div>
              <div className="flex gap-2">
                <input value={symptoms} onChange={(e) => setSymptoms(e.target.value)} placeholder="Describe symptoms (e.g. chest pain, fever...)" className="flex-1 px-3 py-2 rounded-xl outline-none text-sm" style={{ background: '#F8FAFC', border: '1.5px solid #E2E8F0', color: '#0F172A' }}/>
                <button onClick={analyzeSymptoms} className="px-4 py-2 rounded-xl text-white text-sm font-semibold" style={{ background: 'linear-gradient(135deg, #7C3AED, #0EA5E9)' }}>
                  Analyze
                </button>
              </div>
              {suggested && (<div className="mt-2 text-sm font-medium" style={{ color: '#0D9488' }}>
                  ✨ Recommended: {(_a = DEPARTMENTS.find(d => d.id === suggested)) === null || _a === void 0 ? void 0 : _a.name}
                </div>)}
            </div>
            <div className="grid grid-cols-2 gap-3">
              {DEPARTMENTS.map((dept) => (<button key={dept.id} onClick={() => { setSelected(Object.assign(Object.assign({}, selected), { department: dept.id })); setStep(3); }} className="p-4 rounded-2xl text-left transition-all hover:shadow-md bg-white" style={{
                  ...
                  {suggested === dept.id && (<div className="text-xs mt-1 font-medium" style={{ color: '#0D9488' }}>✨ AI Pick</div>)}
                </button>))}
            </div>
          </div>)}
        ...
        {step === 2 && mode === 'bed' && (<div>
            ...
            <div className="flex items-center gap-6 mb-6 p-4 rounded-2xl bg-white" style={{ border: '1.5px solid #E0F2FE' }}>
              {[
            ...
            {[
            ...
            ].map((wardData) => (<div key={wardData.ward} className="mb-6 p-5 rounded-2xl bg-white" style={{ border: '1.5px solid #E0F2FE', boxShadow: '0 2px 12px rgba(0,0,0,0.04)' }}>
                ...
                <div className="flex flex-wrap gap-3">
                  {wardData.beds.map((bed) => {
                    ...
                    return (<button key={bed.id} disabled={!isVacant} onClick={() => isVacant && setSelected(Object.assign(Object.assign({}, selected), { bed: bed.id }))} className="flex flex-col items-center justify-center rounded-xl transition-all" style={{
                        ...
                        <span className="text-xs mt-0.5 capitalize font-medium" style={{ color: isSelected ? 'white' : color }}>
                          {bed.status}
                        </span>
                      </button>);
                })}
            ...
            {selected.bed && (<div className="p-4 rounded-2xl mb-4" style={{ background: '#CCFBF1', border: '1.5px solid #0D9488' }}>
                <div className="flex items-center justify-between">
                  ...
                  <button onClick={() => setStep(3)} className="px-5 py-2 rounded-xl text-white font-semibold text-sm" style={{ background: 'linear-gradient(135deg, #0D9488, #0EA5E9)' }}>
                    Continue →
                  </button>
                </div>
              </div>)}
        ...
        {step === 3 && mode === 'opd' && (<div>
            ...
            <div className="space-y-4">
              {(DOCTORS[selected.department] || DOCTORS.general).map((doc) => {
                var _a;
                return (<div key={doc.id} className="rounded-2xl bg-white overflow-hidden transition-all hover:shadow-lg" style={{ border: '1.5px solid #E2E8F0', boxShadow: '0 2px 12px rgba(0,0,0,0.06)' }}>
                    ...
                    <div className="flex-1 p-4 flex flex-col justify-between">
                      <div>
                        ...
                        <p className="text-sm font-medium mt-0.5" style={{ color: '#0D9488' }}>
                          {(_a = DEPARTMENTS.find(d => d.id === selected.department)) === null || _a === void 0 ? void 0 : _a.name}
                        </p>
                  ...
                  <div className="px-4 py-3 border-t" style={{ borderColor: '#F1F5F9' }}>
                    ...
                    <div className="flex flex-wrap gap-2 mb-3">
                      {doc.slots.map((slot) => (<button key={slot} onClick={() => setSelected(Object.assign(Object.assign({}, selected), { doctor: doc.id, slot }))} className="px-3 py-1.5 rounded-xl text-sm font-medium transition-all" style={{
                    ...
                    <div className="flex gap-3">
                      <input type="date" min={new Date().toISOString().split('T')[0]} onChange={(e) => setSelected(Object.assign(Object.assign({}, selected), { date: e.target.value }))} className="flex-1 px-3 py-2 rounded-xl outline-none text-sm" style={{ background: '#F8FAFC', border: '1.5px solid #E2E8F0', color: '#0F172A' }}/>
                      <button onClick={() => {
                        if (selected.doctor === doc.id && selected.slot && selected.date)
                            setStep(4);
                    }} className="px-4 py-2 rounded-xl text-sm font-semibold transition-all" style={{ background: '#CCFBF1', color: '#0D9488', border: '1.5px solid #0D9488' }}>
                        View Schedule →
                      </button>
                    </div>
                  </div>
        ...
        {/* CONFIRM */}
        {((step === 3 && mode === 'bed') || (step === 4 && mode === 'opd')) && (<div>
            ...
            <div className="rounded-2xl p-5 mb-4 bg-white space-y-3" style={{ boxShadow: '0 2px 16px rgba(0,0,0,0.08)', border: '1.5px solid #F1F5F9' }}>
              {[
                { label: '🏥 Hospital', value: (_b = HOSPITALS.find(h => h.id === selected.hospital)) === null || _b === void 0 ? void 0 : _b.name },
                mode === 'bed'
                    ? { label: '🛏️ Bed', value: selected.bed }
                    : { label: '🏨 Department', value: (_c = DEPARTMENTS.find(d => d.id === selected.department)) === null || _c === void 0 ? void 0 : _c.name },
                mode === 'opd' ? { label: '👨‍⚕️ Doctor', value: (_d = (DOCTORS[selected.department] || DOCTORS.general).find((d) => d.id === selected.doctor)) === null || _d === void 0 ? void 0 : _d.name } : null,
            ...
            ].filter(Boolean).map((item) => (<div key={item.label} className="flex justify-between items-center py-2 border-b border-gray-50 last:border-0">
            ...
            <div className="p-4 rounded-2xl mb-6" style={{ background: '#FFFBEB', border: '1.5px solid #FDE68A' }}>
            ...
            <button onClick={handleBook} disabled={loading} className="w-full py-4 rounded-2xl text-white font-bold text-lg transition-all hover:shadow-xl disabled:opacity-50" style={{ background: 'linear-gradient(135deg, #0D9488, #0EA5E9)' }}>
