...
export default function WhatsAppScanner() {
  ...
  const [activeTab, setActiveTab] = useState<'scan' | 'commands'>('scan')
  ...
  const openWhatsApp = () => {
    window.open(
      `https://wa.me/${TWILIO_WHATSAPP_NUMBER.replace('+', '')}?text=${encodeURIComponent(SANDBOX_JOIN_CODE)}`,
      '_blank'
    )
  }
  ...
  return (
    <div className="rounded-3xl bg-white overflow-hidden"
      style={{ border: '1.5px solid #E0F2FE', boxShadow: '0 4px 24px rgba(0,0,0,0.06)' }}>
        ...
        <div className="p-5">
          ...
          <div className="mt-4 p-3 rounded-2xl flex items-center justify-between"
            ...
            <button
              ...
              style={{ background: copied ? '#CCFBF1' : 'white', color: copied ? '#0D9488' : '#475569', border: '1.5px solid #E2E8F0' }}>
              {copied ? '✅ Copied!' : '📋 Copy'}
            </button>
          </div>
          ...
          <button
            ...
            className="w-full mt-3 py-3 rounded-2xl text-white font-bold text-sm transition-all hover:shadow-lg hover:scale-[1.02] flex items-center justify-center gap-2"
      ...
      {activeTab === 'commands' && (
        <div className="p-5">
          ...
          <div className="space-y-2">
            {COMMANDS.map((item) => (
              <div key={item.cmd}
                className="flex items-center justify-between p-3 rounded-2xl transition-all hover:shadow-sm"
                ...
                <button
                  onClick={() => {
                    window.open(
                      `https://wa.me/${TWILIO_WHATSAPP_NUMBER.replace('+', '')}?text=${encodeURIComponent(item.cmd)}`,
                      '_blank'
                    )
                  }}
                  className="px-3 py-1 rounded-xl text-xs font-semibold transition-all hover:scale-105"
                  style={{ background: '#CCFBF1', color: '#0D9488' }}>
                  Send →
                </button>
              </div>
          ...
          <button
            onClick={openWhatsApp}
            className="w-full mt-4 py-3 rounded-2xl text-white font-bold text-sm transition-all hover:shadow-lg flex items-center justify-center gap-2"
            style={{ background: 'linear-gradient(135deg, #25D366, #128C7E)' }}>
            <span className="text-lg">💬</span>
            Open WhatsApp Chat
          </button>
