...
// ─────────────────────────────────────────────────────────────────────────────
// SESSION STORE  (in-memory — replace with Firebase/Redis for production)
// Tracks each user's current step and collected data
// ─────────────────────────────────────────────────────────────────────────────

type BookingMode = 'opd' | 'bed' | null
...
function getSession(phone: string): Session {
  ...
  const fresh: Session = { step: 'MAIN_MENU', mode: null, data: {}, lastActivity: Date.now() }
  sessions.set(phone, fresh)
  return fresh
}

function setSession(phone: string, update: Partial<Session>) {
  const current = getSession(phone)
  sessions.set(phone, { ...current, ...update, lastActivity: Date.now() })
}
...
export async function sendWhatsApp(toPhone: string, message: string) {
  ...
  try {
    ...
    if (data.sid) { console.log('✅ WA sent:', data.sid); return { success: true, sid: data.sid } }
    console.error('❌ WA failed:', data); return { success: false, error: data }
  } catch (err) {
    console.error('❌ WA error:', err); return { success: false, error: err }
  }
}
...
function departmentMenu() {
  const list = DEPARTMENTS.map((d, i) =>
    `${i + 1}️⃣  ${d.icon} ${d.name}`
  ).join('\n')
  return `🏨 *Select Department:*\n\n${list}\n\n_Reply with a number (1–${DEPARTMENTS.length})_`
}

function doctorMenu(deptId: string) {
...
function slotMenu(deptId: string, doctorIdx: number) {
  const docs = DOCTORS[deptId] || DOCTORS.general
...
export async function handleIncomingWhatsApp(
  fromRaw: string,   // e.g. "whatsapp:+919876543210"
  body: string
) {
  ...
  if (session.step === 'OPD_HOSPITAL') {
    ...
    setSession(phone, { step: 'OPD_DEPARTMENT', data: { ...session.data, hospitalId: hospital.id, hospitalName: hospital.name } })
  ...
  if (session.step === 'OPD_DEPARTMENT') {
    ...
    setSession(phone, { step: 'OPD_DOCTOR', data: { ...session.data, deptId: dept.id, deptName: dept.name } })
  ...
  if (session.step === 'OPD_DOCTOR') {
    ...
    const doc = docs[idx]
    setSession(phone, { step: 'OPD_SLOT', data: { ...session.data, doctorId: doc.id, doctorName: doc.name, doctorIdx: String(idx) } })
  ...
  if (session.step === 'OPD_SLOT') {
    ...
    if (!doc || isNaN(sIdx) || sIdx < 0 || sIdx >= doc.slots.length)
      return sendWhatsApp(phone, `❌ Please reply 1–${doc?.slots.length || 3}.\n\n${slotMenu(session.data.deptId, dIdx)}`)
    const slot = doc.slots[sIdx]
    setSession(phone, { step: 'OPD_DATE', data: { ...session.data, slot } })
  ...
  if (session.step === 'OPD_DATE') {
    ...
    setSession(phone, { step: 'OPD_PATIENT_NAME', data: { ...session.data, date: text } })
  ...
  if (session.step === 'OPD_PATIENT_NAME') {
    ...
    setSession(phone, { step: 'OPD_SYMPTOMS', data: { ...session.data, patientName: text } })
  ...
  if (session.step === 'OPD_SYMPTOMS') {
    ...
    setSession(phone, { step: 'OPD_CONFIRM', data: { ...session.data, symptoms } })
    const d = { ...session.data, symptoms }
  ...
  if (session.step === 'BED_HOSPITAL') {
    ...
    if (isNaN(idx) || idx < 0 || idx >= HOSPITALS.length)
      return sendWhatsApp(phone, `❌ Please reply 1–${HOSPITALS.length}.\n\n${hospitalMenu()}`)
    const hospital = HOSPITALS[idx]
    setSession(phone, { step: 'BED_TYPE', data: { ...session.data, hospitalId: hospital.id, hospitalName: hospital.name } })
  ...
  if (session.step === 'BED_TYPE') {
    ...
    setSession(phone, { step: 'BED_PATIENT_NAME', data: { ...session.data, bedId: bed.id, bedName: bed.name, bedPrice: String(bed.price) } })
    return sendWhatsApp(phone, `👤 *Enter Patient's Full Name:*`)
  }

  if (session.step === 'BED_PATIENT_NAME') {
    if (text.length < 3) return sendWhatsApp(phone, `❌ Please enter a valid name.`)
    setSession(phone, { step: 'BED_MOBILE', data: { ...session.data, patientName: text } })
  ...
  if (session.step === 'BED_MOBILE') {
    ...
    setSession(phone, { step: 'BED_GENDER', data: { ...session.data, mobile: text } })
  ...
  if (session.step === 'BED_GENDER') {
    ...
    setSession(phone, { step: 'BED_AADHAAR', data: { ...session.data, gender: genders[idx] } })
  ...
  if (session.step === 'BED_AADHAAR') {
    ...
    setSession(phone, { step: 'BED_DOCTOR', data: { ...session.data, aadhaar } })
  ...
  if (session.step === 'BED_DOCTOR') {
    ...
    setSession(phone, { step: 'BED_REASON', data: { ...session.data, doctorName } })
  ...
  if (session.step === 'BED_REASON') {
    ...
    setSession(phone, { step: 'BED_DATE', data: { ...session.data, reason: reasons[idx] } })
  ...
  if (session.step === 'BED_DATE') {
    if (!/^\d{2}-\d{2}-\d{4}$/.test(text))
      return sendWhatsApp(phone, `❌ Invalid format. Please reply as *DD-MM-YYYY*`)
    setSession(phone, { step: 'BED_CONFIRM', data: { ...session.data, date: text } })
    const d = { ...session.data, date: text }
...
function opdConfirmMessage(d: Record<string, string>) {
...
function bedConfirmMessage(d: Record<string, string>) {
...
export async function sendAppointmentConfirmation(
  phone: string, name: string, tokenNumber: number,
  hospitalName: string, estimatedWait: number
) {
...
export async function sendYouAreNextAlert(phone: string, name: string, tokenNumber: number) {
...
export async function sendYourTurnNow(
  phone: string, name: string, tokenNumber: number, roomNumber = 'OPD'
) {
...
export async function sendAppointmentReminder(
  phone: string, name: string, tokenNumber: number, hospitalName: string
) {
...
export async function sendFeedbackRequest(phone: string, name: string) {
...
export async function sendTokenSkipped(phone: string, name: string, tokenNumber: number) {
...
export async function sendEmergencyAlert(phone: string, name: string) {
...
export async function sendWelcomeCheckIn(
  phone: string, name: string, hospitalName: string, tokenNumber: number
) {
