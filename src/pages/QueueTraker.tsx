...
export default function QueueTracker() {
  ...
  useEffect(() => {
    const unsubscribe = auth.onAuthStateChanged((firebaseUser) => {
      ...
      onSnapshot(q, (snapshot) => {
        if (!snapshot.empty) {
          setAppointment({ id: snapshot.docs[0].id, ...snapshot.docs[0].data() })
        }
        setLoading(false)
      })
    })
  ...
  const getUrgencyBg = (mins: number) => {
    if (mins <= 5) return '#FEF2F2'
    if (mins <= 15) return '#FFFBEB'
    return '#F0FDFA'
  }

  const getStatusLabel = (status: string) => {
    if (status === 'serving') return '🟢 You are being served now!'
    if (status === 'checkedIn') return '✅ Checked in — waiting for turn'
    if (status === 'waiting') return '⏳ In queue — please wait'
  ...
  return (
    <div style={{ background: '#EFF6FF', minHeight: '100vh' }}>
      ...
      <div className="bg-white sticky top-0 z-50 px-6 py-4 flex items-center gap-4"
        ...
        <div className="ml-auto flex items-center gap-2 px-3 py-1 rounded-full text-xs font-semibold"
      ...
      <div className="max-w-2xl mx-auto px-4 py-8 space-y-6">
        ...
        <div className="rounded-3xl p-8 text-center bg-white"
          ...
          <div className="text-8xl font-bold mb-2"
            style={{ color: getUrgencyColor(wait), fontFamily: 'JetBrains Mono' }}>
            #{token}
          </div>
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full text-sm font-semibold"
            style={{ background: getUrgencyBg(wait), color: getUrgencyColor(wait) }}>
        ...
        <div className="rounded-3xl p-6 text-white"
          style={{ background: 'linear-gradient(135deg, #0D9488, #0EA5E9)', boxShadow: '0 8px 40px rgba(13,148,136,0.3)' }}>
        ...
        {appointment.status === 'booked' && (
          <div className="rounded-3xl p-6 text-center bg-white"
            ...
            <button
              className="px-6 py-3 rounded-2xl text-white font-semibold transition-all hover:shadow-lg"
              style={{ background: 'linear-gradient(135deg, #0D9488, #0EA5E9)' }}>
              📷 Open QR Scanner
            </button>
          </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          <button
            className="py-4 rounded-2xl font-semibold text-sm transition-all hover:shadow-md"
            style={{ background: 'white', color: '#EF4444', border: '1.5px solid #FCA5A5' }}>
            ❌ Cancel Appointment
          </button>
          <button
            className="py-4 rounded-2xl font-semibold text-sm text-white transition-all hover:shadow-md"
            style={{ background: 'linear-gradient(135deg, #0D9488, #0EA5E9)' }}>
            📞 Call Hospital
          </button>
        </div>
