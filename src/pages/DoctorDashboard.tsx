...
export default function DoctorDashboard() {
  ...
  const [currentPatient, setCurrentPatient] = useState<any>(SAMPLE_PATIENTS[0])
  ...
  useEffect(() => {
    const unsubscribe = auth.onAuthStateChanged(async (firebaseUser) => {
      ...
      if (userDoc.exists()) setDoctor(userDoc.data())
      setLoading(false)
    })
    return () => unsubscribe()
  }, [])
  ...
  const handleSkip = () => {
    ...
    setPatients([...rest, { ...currentPatient, status: 'skipped' }])
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              style={{ background: isOnline ? '#0D9488' : '#E2E8F0' }}>
        ...
        <div className="flex items-center justify-between mb-8 flex-wrap gap-4">
          ...
          <div className="flex gap-4">
            {[
              ...
              <div key={stat.label} className="text-center px-5 py-3 rounded-2xl bg-white"
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            {currentPatient ? (
              <div className="rounded-3xl overflow-hidden bg-white"
                style={{ boxShadow: '0 4px 24px rgba(13,148,136,0.12)', border: '2px solid #0D9488' }}>
                ...
                <div className="p-6">
                  <div className="flex items-center gap-4 mb-6">
                    <img src={currentPatient.avatar} alt={currentPatient.name}
                      className="w-20 h-20 rounded-2xl object-cover"
                      style={{ border: '3px solid #CCFBF1' }} />
                    <div>
                      ...
                      <div className="flex items-center gap-2 mt-2">
                        ...
                        <span className="px-3 py-1 rounded-full text-xs font-semibold"
                          style={{ background: '#BAE6FD', color: '#0EA5E9' }}>
                          Normal Priority
                        </span>
                      </div>
                    </div>
                  ...
                  {/* AI Summary */}
                  <div className="p-4 rounded-2xl mb-4"
                    style={{ background: '#DDD6FE', border: '1.5px solid #C4B5FD' }}>
                  ...
                  <textarea
                    ...
                    rows={3}
                    className="w-full px-4 py-3 rounded-2xl outline-none text-sm resize-none mb-4"
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    <button onClick={handleComplete}
                      className="py-3 rounded-2xl text-white font-semibold text-sm transition-all hover:shadow-lg"
                      style={{ background: 'linear-gradient(135deg, #0D9488, #0EA5E9)' }}>
                      ✅ Complete
                    </button>
                    <button onClick={handleSkip}
                      className="py-3 rounded-2xl font-semibold text-sm transition-all hover:shadow-md"
              ...
              <div className="space-y-3">
                {patients.map((patient, index) => (
                  <div key={patient.id}
                    ...
                    style={{
                      background: currentPatient?.id === patient.id ? '#CCFBF1' : '#F8FAFC',
                      border: `1.5px solid ${currentPatient?.id === patient.id ? '#0D9488' : '#E2E8F0'}`
                    }}>
                    ...
                    <div className="text-right flex-shrink-0">
                      ...
                      <div className="w-2 h-2 rounded-full mt-1 mx-auto"
                        style={{ background: patient.checkedIn ? '#0D9488' : '#E2E8F0' }} />
                    </div>
                  </div>
                ))}
